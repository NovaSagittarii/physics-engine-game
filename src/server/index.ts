...
import { Rapier, Wall, Ball } from '../physics';
...
let balls: Map<string, Ball> = new Map();

io.on('connection', (socket) => {
  console.log(socket.id, 'connect');

  {
    const ball = new Ball(world, Math.random() * 10, Math.random() * 10, 1);
...
function gameLoop() {
  ...
  for (const [id, ball] of balls) ball.step();
  world.step();
